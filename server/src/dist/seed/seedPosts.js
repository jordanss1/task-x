...
const fakerPosts = async (uri) => {
    ...
    await Promise.all(todoTasks.map(async ({ task, comments }) => {
        ...
        const awards = taskLikes.likes >= 25 && taskLikes.likes < 50
            ? ["supported"]
        ...
        const created = enabledDueDate
            ? faker.date
                .between({
                from: (0, dayjs_1.default)(dueDate).subtract(3, "weeks").toDate(),
                to: (0, dayjs_1.default)(dueDate).subtract(1, "weeks").toDate(),
            })
                .toISOString()
            : faker.date
                .between({
                from: (0, dayjs_1.default)().subtract(3, "weeks").toDate(),
