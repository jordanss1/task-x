...
const deleteNotificationsForTask_1 = __importDefault(
  require('../functions/deleteNotificationsForTask')
);
...
const taskListRoutes = (app) => {
    ...
    if (publicTask && !onTaskWall) {
      ...
      if (totalTasks?.totalTasks === 1) {
        try {
          ...
          await (0, deleteNotificationsForTask_1.default)(
            req.user._user,
            req.body.taskId
          );
    ...
    if (publicTask && onTaskWall) {
      ...
      if (!totalTasks?.totalTasks || totalTasks.totalTasks === 0) {
        try {
          ...
          res.status(500).send('Edited task but unable add task to task wall');
        }
      } else {
        try {
          await PublicTaskList.findOneAndUpdate(
  ...
  app.post('/api/task/complete', requireJwt_1.default, async (req, res) => {
    ...
    try {
      ...
      return res.status(500).send('Unable to set task as complete, try again');
    }
    if (publicTask) {
      try {
        await PublicTaskList.findOneAndUpdate(
  ...
  app.delete('/api/task', requireJwt_1.default, async (req, res) => {
    ...
    if (publicTasks && publicTasks.totalTasks > 1 && matchingPublicTask) {
      try {
        ...
        res.status(500).send('Unable to delete task from Task Wall, try again');
        return;
      }
    }
    if (publicTasks && publicTasks.totalTasks === 1 && matchingPublicTask) {
